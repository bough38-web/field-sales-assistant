...
if raw_df is not None:
    ...
    try:
        ...
        current_branches = [unicodedata.normalize('NFC', str(b)) for b in target_source['관리지사'].unique() if pd.notna(b)]
                ...
                for i in range(0, len(sorted_branches), GRID_COLS):
                    ...
                    for j in range(GRID_COLS):
                        ...
                        with cols[j]:
                            ...
                            b_source = base_df
                            b_df = b_source[b_source['관리지사'].astype(str).apply(lambda x: unicodedata.normalize('NFC', x)) == b_name]
                            b_total = len(b_df)

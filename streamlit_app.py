...
if raw_df is not None:
    ...
    if active_nav == "🗺️ 지도 & 분석":
        ...
        with st.expander("🗺️ 조건조회", expanded=True):
            ...
            # [FIX] Restore link to dashboard selection
            sel_map_region = st.session_state.get('sb_branch', "전체")
            raw_map_sales = st.session_state.get('sb_manager', "전체")
            
            # Extract name from label if needed (e.g., "G000405 (남기민)" -> "남기민")
            if raw_map_sales != "전체" and "(" in raw_map_sales and ")" in raw_map_sales:
                import re
                match = re.search(r'\(([^)]+)\)', raw_map_sales)
                sel_map_sales = match.group(1) if match else raw_map_sales
            else:
                sel_map_sales = raw_map_sales

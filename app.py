...
from src import ui_components  # [NEW] UI Components for CSS/JS

# [SYSTEM] Configuration & Theme (MUST BE FIRST)
st.set_page_config(
...
GLOBAL_BRANCH_ORDER = ['서대문지사', '중앙지사', '강북지사', '고양지사', '의정부지사', '남양주지사', '강릉지사', '원주지사', '춘천지사', '미지정']
GLOBAL_BRANCH_ORDER = [safe_normalize(b) for b in GLOBAL_BRANCH_ORDER]
...
if 'theme_mode' not in st.session_state:
    st.session_state.theme_mode = "기본 (Default)"
...
# [ROBUSTNESS] Moved get_manager_password to src.utils
# [ROBUSTNESS] Moved mask_name to src.utils

def inject_button_color_script():
    ui_components.inject_button_color_script()
...
with st.sidebar:
        ...
        if data_source == "파일 업로드 (File)":
            if local_zips:
                ...
                if use_local_zip:
                    ...
                    # [UX] Single Master file takes absolute priority
                    preferred_zips = [
                        "LOCALDATA_2026_MASTER.zip",
                        "LOCALDATA_FILTERED_CSV-3월.zip",
                    ...
                    # [UPDATE] Select top priority file
                    default_zips = []
                    for pz in preferred_zips[:1]: # [FIX] Only auto-select the best one to avoid duplicate memory overhead
                        matching = [zip_opts[i] for i, z in enumerate(zip_opts_norm) if z == pz]
                        if matching:
                            default_zips.extend(matching)
                    
                    # [ADD] Included for backwards compatibility, but not auto-appended
                    ...
                    sel_zips = st.multiselect("사용할 인허가 파일 (ZIP)", zip_opts, default=default_zips, help="기본 설정인 'LOCALDATA_2026_MASTER.zip' 단일 사용을 권장합니다.")
    ...
    # [NEW] Daily Status Report Summary (Admin Only)
    if st.session_state.get('user_role') == 'admin':
        with st.sidebar.expander("📢 신규 개업 및 폐업 현황", expanded=False):
            report_path = "data/daily_status_report.json"
            if os.path.exists(report_path):
                try:
                    ...
                    if report_data:
                        # Show latest 5 days in a table
                        latest_reports = report_data[-5:]
                        latest_reports.reverse()
                        
                        st.caption("최근 5일간 수집된 변동 현황")
                        ...
                        # Detailed breakdown for the most recent day
                        if report_data:
                            latest = report_data[-1]
                            st.markdown(f"**상세 ({latest['date']})**")
                            for cat, vals in latest['details'].items():
                                if vals['new'] > 0 or vals['closed'] > 0:
                                    st.write(f"- {cat}: 신규 {vals['new']} / 폐업 {vals['closed']}")
                    else:
                        st.info("수집된 데이터가 없습니다.")
                except Exception as e:
                    st.error(f"리포트 로드 실패: {e}")
            else:
                st.info("리포트 생성 전입니다. 자동 동기화를 실행해 주세요.")
            
            if st.button("📊 리포트 즉시 갱신", key="btn_refresh_report", use_container_width=True):
                with st.spinner("리포트 생성 중..."):
                    try:
                        import subprocess
                        subprocess.run(["python3", "scripts/generate_daily_report.py"], check=True)
                        st.success("갱신 완료")
                        st.rerun()
                    except:
                        st.error("갱신 실패")
...
if raw_df is not None:
    ...
    if '관리지사' in raw_df.columns:
        ...
        def standardize_branch(b):
            ...
            known_branches = ['서대문', '중앙', '강북', '고양', '의정부', '남양주', '강릉', '원주', '춘천']
    ...
    # [GLOBAL INITIALIZATION] Bulk Load activity status for all views (Map, Sidebar, Grid)
    if raw_df is not None:
        try:
            # 1. Initialize record_key if missing
            if 'record_key' not in raw_df.columns:
                temp_names = raw_df['사업장명'].fillna("").astype(str)
                temp_addrs = raw_df['소재지전체주소'].fillna("").astype(str)
                raw_df['record_key'] = [utils.generate_record_key(n, a) for n, a in zip(temp_names, temp_addrs)]
            
            # 2. Load latest statuses
            status_data = activity_logger.load_json_file(activity_logger.ACTIVITY_STATUS_FILE)
            if not isinstance(status_data, dict): status_data = {}
            
            # 3. Create mapping and apply to raw_df
            ...
            raw_df['활동진행상태'] = raw_df['record_key'].map(status_map).fillna('').astype(str)
            # Normalize status (Emojis)
            raw_df['활동진행상태'] = raw_df['활동진행상태'].apply(activity_logger.normalize_status)
            
            # [NEW] Merge Activity Date into '최종수정시점'
            # If '최종수정시점' missing, initialize from License/Closure (already done in data_loader)
            if '최종수정시점' not in raw_df.columns:
                # Fallback if loader didn't do it
                d_open = pd.to_datetime(raw_df['인허가일자'], errors='coerce') if '인허가일자' in raw_df.columns else pd.Series(pd.NaT, index=raw_df.index)
                d_close = pd.to_datetime(raw_df['폐업일자'], errors='coerce') if '폐업일자' in raw_df.columns else pd.Series(pd.NaT, index=raw_df.index)
                raw_df['최종수정시점'] = pd.concat([d_open, d_close], axis=1).max(axis=1)
            else:
                 # Ensure it is datetime
                 raw_df['최종수정시점'] = pd.to_datetime(raw_df['최종수정시점'], errors='coerce')

            # Now update with Activity Date (Priority)
            activity_dates = pd.to_datetime(raw_df['record_key'].map(date_map), errors='coerce')
            raw_df['최종수정시점'] = pd.concat([raw_df['최종수정시점'], activity_dates], axis=1).max(axis=1)
            
            # Final Fallback to current time if still NaT (extremely old data without dates)
            raw_df['최종수정시점'] = raw_df['최종수정시점'].fillna(utils.get_now_kst())
            
        except Exception as e:
            # Fallback to empty column if everything fails
            raw_df['활동진행상태'] = ''
            if '최종수정시점' not in raw_df.columns:
                raw_df['최종수정시점'] = utils.get_now_kst()

        raw_df['관리지사'] = raw_df['관리지사'].apply(standardize_branch)
    else:
        raw_df['관리지사'] = '미지정'

    # [FIX] Global NFC Normalization (Safe)
    for col in ['관리지사', 'SP담당', '사업장명', '소재지전체주소', '영업상태명', '업태구분명']:
        if col in raw_df.columns:
            raw_df[col] = raw_df[col].apply(safe_normalize)
    ...
    if not raw_df.empty:
        ...
        # 2. Standardize Date Columns to be Timezone-Naive (KST)
        for col in ['인허가일자', '폐업일자', '최종수정시점', '데이터기준일자']:
            if col in raw_df.columns:
                # [FIX] Force timezone-naive KST for all date columns to prevent comparison issues
                temp_dt = pd.to_datetime(raw_df[col], errors='coerce', utc=True)
                raw_df[col] = temp_dt.dt.tz_convert('Asia/Seoul').dt.tz_localize(None)
    ...
    if raw_df is not None and not raw_df.empty:
        current_branches_raw = [safe_normalize(b) for b in raw_df['관리지사'].unique() if pd.notna(b)]
    ...
    with st.sidebar:
        ...
        if raw_df is not None and not raw_df.empty:
            with st.spinner("🔍 데이터를 정밀 분석하고 점수를 계산하는 중..."):
    ...
    # [OPTIMIZATION] '최종수정시점' is now pre-calculated and merged with activity dates in global init
    ...
    if st.session_state.user_role == 'branch':
        if st.session_state.user_branch:
             ...
             u_branch = safe_normalize(st.session_state.user_branch)
    ...
    if custom_view_mode and admin_auth and (custom_view_managers or exclude_branches):
        ...
        # [FIX] Synchronize dashboard with sidebar selection for all roles.
        # This ensures the '지사별 현황' cards match the managers shown below.
        current_branch_filter = st.session_state.get('sb_branch', "전체")
        
        if current_branch_filter != "전체":
            # [FIX] Normalize comparison for Mac/Excel compatibility
            norm_sel_branch = safe_normalize(current_branch_filter)
        ...
        if selected_area_code:
            ...
            norm_sel_manager = safe_normalize(sel_manager)
    ...
    if isinstance(g_range, (list, tuple)) and len(g_range) == 2:
        ...
        if '최종수정시점' in base_df.columns:
             ...
             # [FIX] Ensure '최종수정시점' is timezone-naive KST for strict comparison
             if base_df['최종수정시점'].dt.tz is not None:
                  base_df['최종수정시점'] = base_df['최종수정시점'].dt.tz_convert('Asia/Seoul').dt.tz_localize(None)
             
             # [FIX] Ensure boundary consistency (KST Naive)
    ...
    if address_search:
        ...
        search_norm = safe_normalize(address_search)
    ...
    if edit_mode:
        ...
        base_cols.extend(['소재지전체주소', '업태구분명', '인허가일자', '최종수정시점'])
        ...
        st.stop() # Stop further rendering in edit mode
    ...
    try:
        # [FIX] For Dashboard, allow seeing other branches if selected, consistent with Manager Section.
        if sel_branch != "전체":
             target_source = raw_df
        else:
             target_source = base_df # Maintain security lockdown if 'All' is selected
             
        current_branches = [safe_normalize(b) for b in target_source['관리지사'].unique() if pd.notna(b)]
    ...
    # [FEATURE] Local AI Performance/Insight Logic
    if not df.empty:
        branch_stats = df.groupby('영업상태명').size().to_dict()
        total = sum(branch_stats.values())
        active = branch_stats.get('영업/정상', 0)
        closed = branch_stats.get('폐업', 0)
        active_rate = (active / total * 100) if total > 0 else 0
        
        st.markdown("#### 🤖 AI 영업 리포트 - 지사별 성과 요약")
        c1, c2, c3 = st.columns(3)
        with c1:
            st.metric("총 활동 대상", f"{total:,}건")
        with c2:
            st.metric("신규/유효 영업", f"{active:,}건", delta=f"{active_rate:.1f}%")
        with c3:
            st.metric("폐업 관리", f"{closed:,}건")
    ...
    if st.session_state.user_role != 'manager':
        with st.expander("🏭 지사별 현황", expanded=True):
            ...
            # [STABILITY FIX] Use a fixed 4-column grid instead of one giant dynamic row
            # This prevents the 'NotFoundError: removeChild' error common in Streamlit dynamic layouts.
            if not sorted_branches:
                ...
                sel_dashboard_branch = safe_normalize(raw_dashboard_branch)

                for i in range(0, len(sorted_branches), GRID_COLS):
                    cols = st.columns(GRID_COLS)
                    for j in range(GRID_COLS):
                        if i + j >= len(sorted_branches):
                            break
                            
                        b_name = sorted_branches[i + j]
                        with cols[j]:
                            # 1. Calculate Stats 
                            # [FIX] Force all roles (including Admin) to use base_df for Branch Cards
                            # This ensures that the cards reflect the Global Date Range filter.
                            b_source = base_df
                            b_df = b_source[b_source['관리지사'].astype(str).apply(safe_normalize) == b_name]
                            b_total = len(b_df)
    ...
    if not base_df.empty:
        ...
        current_br_name = safe_normalize(current_br_name)
        ...
        with st.expander(f"👤 {current_br_name} 영업담당 현황", expanded=True):
            ...
            if current_br_name != "전체":
                ...
                mgr_df = raw_df[raw_df['관리지사'].astype(str).apply(safe_normalize) == current_br_name].copy()
                ...
                # 2. Re-apply ALL Filters (Date Range, Month, Type, Status)
                # [FIX] Added Global Date Range Filter to Manager Summary
                g_r = st.session_state.get('global_date_range', ())
                if isinstance(g_r, (list, tuple)) and len(g_r) == 2:
                    ts_s = pd.Timestamp(g_r[0]).normalize()
                    ts_e = pd.Timestamp(g_r[1]).normalize() + pd.Timedelta(hours=23, minutes=59, seconds=59)
                    if '최종수정시점' in mgr_df.columns:
                        mgr_df = mgr_df[(mgr_df['최종수정시점'] >= ts_s) & (mgr_df['최종수정시점'] <= ts_e)]
                ...
                if only_with_phone:
                    mgr_df = mgr_df[mgr_df['소재지전화'].notna() & (mgr_df['소재지전화'] != "")]
                if only_hospitals:
    ...
    if active_nav == "🗺️ 지도 & 분석":
        ...
        with st.expander("🗺️ 조건조회", expanded=True):
            ...
            with c_q_r2_3: st.empty() # Placeholder after removing 정지시설
            ...
            # Avoid KeyError: check if lat/lon exist first
            if 'lat' in base_df.columns and 'lon' in base_df.columns:
                map_df_base = base_df.dropna(subset=['lat', 'lon']).copy()
            else:
                st.warning("⚠️ 지도 표시를 위한 좌표 정보(lat, lon)가 데이터에 포함되어 있지 않습니다.")
                map_df_base = pd.DataFrame()
        ...
        # [OVERHAUL] Pre-calculate record_key for Map (High Performance)
        # This ensures the key sent from Map matches the key used in Grid
        # [FIX] Replaced slow df.apply() with vectorized-equivalent list comprehension 
        if not map_df.empty:
            temp_names = map_df['사업장명'].fillna("").astype(str)
            temp_addrs = map_df['소재지전체주소'].fillna("").astype(str)
            map_df['record_key'] = [utils.generate_record_key(n, a) for n, a in zip(temp_names, temp_addrs)]
